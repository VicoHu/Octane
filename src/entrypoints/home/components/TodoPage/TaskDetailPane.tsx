...
function IconButton({
  ...
  return (
    <Tooltip>
      ...
      >
        <span data-icon="inline-start">{children}</span>
      </TooltipTrigger>
  ...
  if (!editable)
    return (
      <div className={styles.checklistItem}>
        <Checkbox aria-label={`完成${item.text}`} checked={item.isCompleted} disabled /> <span>{item.text}</span>
      </div>
    );
  return (
    <div className={styles.checklistItem}>
      <Checkbox
        ...
        onCheckedChange={(checked) => onComplete(item.id, Boolean(checked))}
      />
      <Input
        aria-label={`检查项${item.text}`}
        value={text}
...
export const TaskDetailPane = forwardRef<TaskDetailPaneHandle, TaskDetailPaneProps>(function TaskDetailPane(
  ...
  useImperativeHandle(ref, () => ({ commitDraft, discardDraft, hasDirtyDraft }), [
    commitDraft,
    discardDraft,
    hasDirtyDraft,
  ]);
  ...
  const openMove = () => {
    ...
    setMoveLoading(true);
    setMoveOpen(true);
    void loadMoveOptions(today)
      .then(setMoveOptions)
      .catch((reason) => setMoveError(errorMessage(reason, "读取移动目标失败")))
  ...
  const changeTags = (tagId: string, checked: boolean) => {
    ...
    void runPending(
      ...
      async () => {
        try {
          await replaceTaskTags(task.id, next);
          setTagOverride(null);
        } catch (reason) {
          setTagOverride(null);
          throw reason;
  ...
  const updateChecklist = (itemId: string, text: string) =>
    runPending(`check-text-${itemId}`, () => updateChecklistItem(itemId, text), "更新检查项失败");
  const removeTask = () => {
    if (!task) return;
    void (async () => {
    ...
    return (
      <section className={styles.detailPane} aria-label="待办详情">
        <header className={styles.paneHeader}>
          {mobile && (
            <IconButton label="返回列表" className={styles.mobileDetailAction} onClick={onBack}>
              <ArrowLeft />
            </IconButton>
          )}
          <h2>详情</h2>
  ...
  return (
    <section className={styles.detailPane} aria-label="待办详情">
      <header className={styles.paneHeader}>
        ...
        <h2>详情</h2>
        {savingDraft && <Spinner aria-label="保存中" />}
      </header>
      <div className={styles.detailBody}>
        {draftError && (
          <div className={styles.draftError} role="alert">
            {draftError}
            <Button size="sm" variant="outline" onClick={() => void commitDraft()}>
              重试
            </Button>
        ...
        {archived && detail.taskList && (
          <Button
            variant="outline"
            disabled={isPending("list-restore")}
            onClick={() => void runPending("list-restore", () => restoreTaskList(detail.taskList!.id), "恢复清单失败")}
          >
            <RotateCcw data-icon="inline-start" />
            恢复清单
          </Button>
        )}
        <section className={styles.checklistSection} aria-label="检查项">
          <h3>检查项</h3>
          {!readOnly && (
            <div className={styles.addChecklist}>
              ...
              <Button
                size="sm"
                disabled={checklistCreating || newChecklistText.trim() === ""}
                onClick={() => void addChecklist()}
              >
                {checklistCreating ? <Spinner aria-label="创建检查项" /> : "添加"}
              </Button>
            </div>
          )}
          {detail.checklistItems.length === 0 ? (
            <p className={styles.emptyChecklist}>暂无检查项</p>
          ) : (
            detail.checklistItems.map((item) => (
              <ChecklistRow
                ...
                pending={{
                  completion: isPending(`check-complete-${item.id}`),
                  text: isPending(`check-text-${item.id}`),
                  reorder: isPending("check-reorder"),
                  delete: isPending(`check-delete-${item.id}`),
                }}
                onComplete={(itemId, completed) =>
                  void runPending(
                    `check-complete-${itemId}`,
                ...
                onDelete={(itemId) =>
                  void runPending(`check-delete-${itemId}`, () => deleteChecklistItem(itemId), "删除检查项失败")
                }
              />
            ))
          )}
        </section>
        <section className={styles.systemFields} aria-label="系统信息">
      ...
      {moveOpen && (
        <TaskMoveDialog
          key={moveOptions ? `ready-${task.id}` : `loading-${task.id}`}
          open
          onOpenChange={setMoveOpen}
          task={task}
          sourceTags={detail.taskTags}
          workspaces={workspaces}
          groups={moveOptions?.groups ?? []}
          loading={moveLoading}
          loadError={moveError}
          moveTask={moveTask}
        />
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setCompletionConfirm(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>完成待办</AlertDialogTitle>
            <AlertDialogDescription>还有 {completionConfirm ?? 0} 个未完成检查项。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction onClick={() => void toggleCompletion(true, true)}>
