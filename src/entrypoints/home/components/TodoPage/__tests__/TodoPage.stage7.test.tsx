...
describe("QuickAddTask — 上下文继承", () => {
  ...
  it("未来七天默认今天截止日期且在所有工作区显示工作区选择", async () => {
    ...
    await waitFor(() =>
      expect(createTask).toHaveBeenCalledWith(
        expect.objectContaining({ workspaceId: "w1", listId: null, dueDate: "2026-08-20", title: "准备材料" }),
      ),
    );
  });
});
...
describe("TaskListPane — 完成、删除与快捷创建", () => {
  it("未完成检查项要求确认后才完成待办", async () => {
    ...
    useTodoData.setState({
      queryResult: { active: [row()], completed: [], total: 1, effectiveSort: "manual" },
      setTaskCompletion,
    } as never);
    ...
    await waitFor(() =>
      expect(setTaskCompletion).toHaveBeenLastCalledWith("t1", true, { allowIncompleteChecklist: true }),
    );
  });
  ...
  it("完成选中任务后 active-only 移除行并提供撤销", async () => {
    ...
    const setTaskCompletion = vi
      .fn()
      .mockResolvedValue({ status: "updated", task: { ...row().task, status: "completed" } });
    useTodoData.setState({
      queryResult: { active: [row()], completed: [], total: 1, effectiveSort: "manual" },
      setTaskCompletion,
    } as never);
  ...
  it("删除选中任务后优先选择下一行并提供恢复", async () => {
    ...
    useTodoData.setState({
      queryResult: {
        active: [row(), row({ id: "t2", task: { ...row().task, id: "t2", title: "下一条" } })],
        completed: [],
        total: 2,
    ...
    expect(toast.success).toHaveBeenCalledWith(
      expect.objectContaining({ action: expect.objectContaining({ label: "恢复" }) }),
    );
  });
  ...
  it("删除后切到废纸篓能看到该待办（hiddenIds 不跨视图泄漏）", async () => {
    ...
    useTodoData.setState({
      queryResult: { active: [deletedRow], completed: [], total: 1, effectiveSort: "manual" },
    } as never);
  ...
  it("废纸篓行菜单为「恢复待办 / 永久删除」，永久删除需二次确认", async () => {
    ...
    useTodoData.setState({
      queryResult: {
        active: [row({ task: { ...row().task, deletedAt: 100 } })],
        completed: [],
        total: 1,
        effectiveSort: "manual",
      },
  ...
  it("恢复失败时回滚隐藏并报错，不弹成功 Toast", async () => {
    const user = userEvent.setup();
    const restoreTask = vi.fn().mockRejectedValue(new Error("boom"));
    useTodoData.setState({
      queryResult: {
        active: [row({ task: { ...row().task, deletedAt: 100 } })],
        completed: [],
    ...
    render(<TaskListPane onOpenNavigation={vi.fn()} />);
    await user.click(screen.getByRole("button", { name: "整理发布更多操作" }));
    await user.click(await screen.findByRole("menuitem", { name: "恢复待办" }));

    // 失败后行重新可见、报错，且没有「待办已恢复」的成功 Toast
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith("恢复待办失败"));
    expect(screen.getByText("整理发布")).toBeInTheDocument();
    expect(toast.success).not.toHaveBeenCalledWith(expect.objectContaining({ content: "待办已恢复" }));
  });

  it("永久删除失败时报错，不崩溃", async () => {
    const user = userEvent.setup();
    const deleteTaskPermanently = vi.fn().mockRejectedValue(new Error("boom"));
    useTodoData.setState({
      queryResult: {
        active: [row({ task: { ...row().task, deletedAt: 100 } })],
        completed: [],
    ...
    await waitFor(() => {
      expect(deleteTaskPermanently).toHaveBeenCalledWith("t1");
      expect(toast.error).toHaveBeenCalledWith("永久删除待办失败");
    });
  });
});
