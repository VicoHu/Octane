...
function isCurrentViewAvailable(
  ...
  const list = navigation.groups.flatMap((group) => group.taskLists).find((candidate) => candidate.id === view.listId);
...
export function TodoPage({ active, activePage, onNavigate, onRegisterLeaveGuard }: TodoPageProps) {
  ...
  const applySplitLayout = useCallback(
    (percent: number, persist: boolean) => {
  ...
  useEffect(() => {
    let mounted = true;
    void loadTodoUiPreferences()
      .then(({ detailSplitPercent }) => {
  ...
  const selectTaskWithDraft = useCallback(
    (taskId: string) =>
      runAfterDraft(() => {
  ...
  const changeViewWithDraft = useCallback(
  ...
  const changeScopeWithDraft = useCallback(
    (nextScope: "current" | "all") =>
      runAfterDraft(() => {
  ...
  const navigateFromTodo = useCallback(
    (page: AppPage) => {
      if (page === "tasks") {
        onNavigate(page);
        return;
    ...
    >
      ...
      {isMobile ? (
        mobileDetailOpen ? (
          ...
          <TaskListPane
            onOpenNavigation={() => setTodoNavOpen(true)}
            onTaskSelect={selectTaskWithDraft}
            onBeforeTaskDelete={commitBeforeTaskDelete}
          />
        )
      ) : (
        <ResizablePanelGroup
        ...
        >
          ...
          <ResizablePanel id="task-detail" minSize={minDetailSize}>
            <TaskDetailPane
              ...
              mobile={false}
              onBack={() => undefined}
              onClose={() => void runAfterDraft(() => useTodoView.getState().selectTask(null))}
            />
          </ResizablePanel>
        </ResizablePanelGroup>
      )}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>无法保存当前标题或描述。请重试，或放弃修改后继续。</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>留在当前待办</AlertDialogCancel>
            <AlertDialogAction
              variant="outline"
              onClick={() =>
                void (async () => {
                  if (await detailRef.current?.commitDraft()) {
            ...
            <AlertDialogAction
              variant="destructive"
              onClick={() => {
                detailRef.current?.discardDraft();
                const action = blockedAction;
                setBlockedAction(null);
                void action?.();
              }}
