...
- **快速解锁 PIN（#96）** —— 可选的第二层解锁因子，解决「主密码太长、日常解锁繁琐」：设置中心「主密码」页新增「快速解锁 PIN」分区，支持 4 位数字 / 6 位数字 / 自定义（≥4 位任意字符）三档。主密码保持 ≥12 字符强度要求、仍是唯一加密根，PIN 只用于解锁；启用 / 修改 / 关闭 PIN 均需主密码确认（防离席劫持）。PIN 经 PBKDF2-SHA256（600,000 迭代、独立随机盐）派生 KEK 包装派生密钥成「信封」——主密钥永不明文持久化（对标 Bitwarden AFU/BFU 模式）。
  - **启用流程为两步向导**：第 1 步验证主密码（完整 PBKDF2 + verifier 校验，密码验证即焚），第 2 步选择形态（RadioGroup：6 位推荐 / 4 位 / 自定义，附推荐语）并设置，支持「上一步」返回；关闭 PIN 保持单步。多步带步骤指示（aria-current）、异步提交防双击、错误就近显示、输入均 label 关联。4 位 / 6 位数字形态的 PIN 输入为分格方框（InputOTP，每位一格自动跳格、支持整段粘贴），设置弹窗内明文展示、解锁弹窗保持掩码。
  - **重启保持（默认关）**：「重启浏览器后仍可用 PIN 解锁」独立开关，开启后信封持久化到 `chrome.storage.local`，日常彻底告别重启重输主密码；开启时与设置页均明示离线暴力破解风险（4 位 PIN 仅 1 万种组合），弱组合（4 位 PIN + 永不自动锁定 + 重启保持）触发最强警示文案。
  ...
  - **修改主密码联动**：改密弹窗在 PIN 启用时要求填写当前 PIN（先解旧信封验证，输错则整个改密原子失败），信封随新密钥自动重建（改密不必重设 PIN）。
...
- **标签会话冷恢复（持续保存 + 冷启动恢复）** —— 开启工作区标签隔离（close / hide-discard / hide）后，当前工作区的标签页会被持续保存到本机。即使不切换工作区直接关闭浏览器，下次冷启动也会在 Chrome 原生会话稳定后非破坏式恢复，不再依赖 Chrome 是否开启「继续浏览上次打开的网页」。此前关闭前未切换工作区则最新标签页丢失、且恢复完全交给 Chrome 原生机制的问题得以修复。

  - **持续保存**：运行期间持续维护当前工作区的最新标签快照，并单独记录关闭前的单窗口拓扑（当前工作区 + 仍以折叠组驻留的其他工作区）；标签增删、改 URL、移动、固定状态、入/离组等变化去抖落盘，陈旧异步写不覆盖最新快照。
  - **冷启动协调（非破坏式 multiset 匹配）**：等 Chrome 原生标签拓扑稳定后做多重集匹配——复用 Chrome 已恢复的标签页，只补齐缺失的数量，保留快照外的标签页，不接管、不移动、不关闭用户自建标签组；有意重复的 URL 保持重复（两个相同 URL 不被合并成一个）。仅查当前窗口，不用 `chrome.sessions` 作为真值（其最近关闭限制与持久归属不匹配）。
  - **按隔离模式恢复**：当前工作区恢复为散开标签页（保持已有切换模型）；关闭前仍驻留的其他工作区恢复为折叠的 Octane 标签组（hide / hide-discard）；close 模式仅恢复当前工作区，非活跃工作区保持关闭。旧版本无拓扑记录时保守迁移：只恢复合法的当前工作区。
  - **Home 保持激活 + 业务标签延迟加载**：恢复前先确保 Octane Home 激活，缺失标签以非激活方式创建（不抢焦点、不 discard），用户聚焦时才加载——避免启动时触发网络 / 内存 / 音频 / 登录风暴。冷启动不承诺任何模式的页面运行时状态。
  - **失败容错**：每个恢复项独立成败；失败项自动重试一次；仍失败的在 Home 给出一次性数量通知（不泄露 URL）与手动重试入口，失败项保留到恢复成功或后续显式归档建立新基线（不把部分失败静默转成数据丢失）。
  - **隐私边界**：标签会话、拓扑、恢复令牌与待恢复项仅存 `chrome.storage.local`，排除在备份 / 云同步（WebDAV / S3）/ 分享包之外；删除工作区同步清理其会话与拓扑引用；日志与反馈只报数量与工作区名，不含完整 URL。无新增 Chrome 权限（复用既有 `tabs` / `tabGroups` / `storage`）。
  - **幂等与一次性**：仅 `runtime.onStartup` 触发一次冷恢复，扩展安装 / 更新 / service worker 唤醒 / Home 重载 / 新建窗口都不再触发；恢复期间挂起自动保存，避免不完整中间态覆盖最后完整快照。
  - **作用域**：支持单个正常 Chrome 窗口的冷恢复；多窗口与隐身窗口冷恢复不在本期范围（多窗口时 fail-closed，保留原生标签状态不动）。

- **Sidebar「添加分类」入口上移**：原底部全宽「添加分类」按钮移到「分类」标题行右侧，改为与「常驻书签」一致的 shadcn ghost `Plus` 图标按钮（Tooltip + `aria-label="添加分类"` + 可见聚焦环）；工作区切换或分类重排期间禁用，防止建错工作区；空分类提示改为「点 + 添加分类」；底部「管理」「设置」保持原位，既有新建分类弹窗与流程不变。

### Internal

- 修复 WXT 导入 background 时 fake-browser 未实现 listener 导致的构建失败：仅跳过 fake-browser 特征错误，其他注册异常继续抛出。
- recovery 全链路 TDD：`sessionContinuity` 用 Map 驱动的 fake Chrome Adapter + 真实 `storage.local` 往返，承重用例复现「A 驻留 / B 新增 / 关浏览器 / 冷启动恢复」全序列；真机核验三轮修复——移除冷恢复 `discardTab` 改 `createTab({active:false})`（discard 未加载 tab 真机静默损坏）、大幅增加原生拓扑稳定等待（避与 Chrome session restore 竞态致 tab 被吞）、恢复顺序改为组在前散开在后并清理原生新标签页噪声。
